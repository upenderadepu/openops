...
import { isObject, StepOutput } from '@openops/shared';
...
function removeLeavesInTopologicalOrder(
  ...
  nodes
    .filter((node) => node.numberOfChildren === 0)
    .forEach((node) => leaves.add(node));
...
function traverseJsonAndConvertToNodes(root: unknown) {
  ...
  while (!nodesQueue.isEmpty()) {
    ...
    nodes.push({
      ...
      size:
        children.length === 0
          ? sizeof(curNode.value)
          : children.length * SIZE_OF_TRUNCATION_TEXT_PLACEHOLDER,
      key: curNode.key,
      parent: {
    ...
    children.forEach((child) => {
      ...
      nodesQueue.enqueue({
        value,
        key,
        parent: { index: nodes.length - 1, value: curNode.value },

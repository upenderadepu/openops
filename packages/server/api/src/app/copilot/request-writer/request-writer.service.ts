...
function getOpenAI(): OpenAI {
  ...
  const openaiInstanceType = system.getOrThrow<CopilotInstanceTypes>(
    AppSystemProp.COPILOT_INSTANCE_TYPE,
  );

  switch (openaiInstanceType) {
    case CopilotInstanceTypes.AZURE_OPENAI: {
      const apiVersion = system.getOrThrow(
        AppSystemProp.AZURE_OPENAI_API_VERSION,
      );
      openai = new OpenAI({
        apiKey,
...
export const requestWriterService = {
  async generateSearchQuery({ prompt }: RequestWriteParams): Promise<string> {
    ...
    const extractedSearchQuery = await openAI.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: customGPTPrompt },
    ...
    const searchQuery =
      extractedSearchQuery.choices[0].message.content.trim() as string;
    return searchQuery;
  },
  ...
  async fetchAndProcessURLs(searchQuery: string): Promise<string[]> {
    ...
    const htmlResults = await Promise.all(
      urls.map((url) => axios.get(url, { headers: { Accept: '*/*' } })),
    );
    const markdownResults = await Promise.all(
      htmlResults.map((res) => HtmlToMd(res.data)),
    );
  ...
  async generateHttpRequest({ prompt }: RequestWriteParams): Promise<string> {
    ...
    const extractedJsonRequest =
      completion.choices[0].message.content.trim() as string;
  ...
  async createRequest({ prompt }: RequestWriteParams): Promise<string> {
    ...
    const httpRequestDetails = await this.generateHttpRequest({
      prompt: apiReferencePrompt,
    });
